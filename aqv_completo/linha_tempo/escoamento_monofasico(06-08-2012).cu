...
// 		
// 		int k;
// 		int temp = tam_mat_real * tam_mat_real;
// 		float somaSerial = 0.0, M = 0.0;
// 		for(k = 0; k < tam_mat_real; k ++)
// 			for(j = 0; j < tam_mat_real; j++)
// 				somaSerial += host_aux_1[k*tam_mat_real + j];
// 			
// 		M = somaSerial / (tam_mat_interna * tam_mat_interna);
...
// 		int num_blocks, qtElementos;
// 		qtElementos = tam_mat_real*tam_mat_real;
// 		num_blocks = ((qtElementos) > 16) ? (qtElementos/16) + 1 : 1;
// 			
// 		reduce1<<<num_blocks, 16>>>( dev_pressao.p, dev_media, qtElementos );
// 		
// 		HANDLE_ERROR( cudaMemcpy( host_aux_1, dev_media, tam_mat_real * tam_mat_real * sizeof(float),
//                               cudaMemcpyDeviceToHost ) );
// 		
...
// 		for(k = 0; k < tam_mat_real; k ++){
// 			for(j = 0; j < tam_mat_real; j++)
// 				printf("%16.6E ", host_aux_1[k*tam_mat_real + j]);
// 			printf("\n");
// 		}
// 
// 
// 
// 		float somaCuda = 0.0, resultadoCuda = 0.0;
// 		for(k = 0; k < num_blocks; k++)
// 			somaCuda += host_aux_1[k];
// 		
// 		resultadoCuda = somaCuda / (tam_mat_interna * tam_mat_interna);
// 		printf("valor da media em cuda = %f \t i = %d", resultadoCuda , i);
// 		
// 		HANDLE_ERROR( cudaMemcpy( host_aux_1, dev_pressao.p, tam_mat_real * tam_mat_real * sizeof(float),
//                               cudaMemcpyDeviceToHost ) );
// 		printf("\n\n\t\t\t\tpressao:\n");
...
// 															M);
// 		
// 		teste<<<block, thread>>>( dev_pressao, dev_q, dev_l );
