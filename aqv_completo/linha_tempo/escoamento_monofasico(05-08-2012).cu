...
// 	dim3 thread(16, 16);
	...
	while (i < 10){
		...
		float soma = 0.0, M = 0.0;
		for(k = 0; k < temp; k++){
			soma = soma + host_aux_1[k];
		}
		M = soma / (tam_mat_interna * tam_mat_interna);
		...
		reduce1<<<num_blocks, 512>>>( dev_pressao.p, dev_media, qtElementos );
		...
		M = soma / (tam_mat_interna * tam_mat_interna);
		printf("valor da media em cuda = %f \t i = %d", M , i);
