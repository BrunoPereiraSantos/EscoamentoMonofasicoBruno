...
__global__ void escoamento_monofasico(	ESTRUTURA_Q dev_q,
	...
	*/
	__syncthreads();
	float media = 0.0, dev_sum1 = 0.0, dev_sum2 = 0.0, erro = 0.0;
	...
	dev_aux[tid] = dev_pressao.p[tid] - dev_pressao.p_old[tid];
	__syncthreads();
	if(tid == eq_tid_cant_inf_dir){
		int i = 0;
		for(i = 0; i <= eq_tid_cant_inf_dir; i++){
...
// 	dim3 thread(16, 16);
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf("%1.e ", host_b.L[i*tam_mat_real + j]);
		printf("\n");
	}
	printf("\n---------------------------------------------------------------------\n");
	printf("\nq_U:\n");
