...
// 		    erro = sqrt(sum1/sum2);
	...
	dev_pressao.p[tid] -= dev_media[0];
	dev_l.D[tid] -= dev_media[0];
	dev_l.U[tid] -= dev_media[0];
	dev_l.L[tid] -= dev_media[0];
	dev_l.R[tid] -= dev_media[0];
...
__global__ void teste(	ESTRUTURA_PRESSAO dev_pressao,
												ESTRUTURA_Q dev_q,
												ESTRUTURA_L dev_l
												){
...
char inicializa_parametros(){
	...
	HANDLE_ERROR( cudaMemset( dev_aux_1, 0, tam_mat_real * tam_mat_real * sizeof(float) ) );
	HANDLE_ERROR( cudaMemset( dev_aux_2, 0, tam_mat_real * tam_mat_real * sizeof(float) ) );
