...
/* - - - - - - - Ponteiros para GPU  - - - - - - - */
float *dev_aux_1 = NULL, *dev_aux_2 = NULL, dev_erro = NULL, *dev_media =  NULL;
...
__global__ void prepara_criterio_convergencia(float *dev_aux_1, 
											   float *dev_aux_2,
											   float *dev_media, 
											   ESTRUTURA_Q dev_q, 
											   ESTRUTURA_L dev_l, 
...
// 		    erro = sqrt(sum1/sum2);
	...
	dimensao_x -= 2;
	dimensao_y -= 2;
	//float media = dev_media[0] / (dimensao_x * dimensao_y);
	float media = dev_media[0] / (dimensao_x * dimensao_y);
...
// 			printf("\n");
// 		}
// 		memset (host_b.R, 0.0, tam_mat_real * tam_mat_real * sizeof(float));
		
		
		
		prepara_criterio_convergencia<<<block, thread>>>( 	dev_aux_1, 
															dev_aux_2,
															dev_media, 
															dev_q, 
															dev_l, 
