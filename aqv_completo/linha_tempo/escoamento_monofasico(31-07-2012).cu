...
__global__ void escoamento_monofasico(	ESTRUTURA_Q dev_q,
										...
										ESTRUTURA_PRESSAO dev_pressao,
										ESTRUTURA_MAT dev_mat,
										float *dev_aux_1, const float erro_max, float dev_erro, float *dev_media){
    /*int x = threadIdx.x + blockIdx.x * blockDim.x;
    int y = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void perapara_criterio_convergencia(float *dev_aux_1, 
											   float *dev_aux_2,
											   float *dev_media, 
											   ESTRUTURA_Q dev_q, 
											   ESTRUTURA_L dev_l, 
...
__global__ void teste( ESTRUTURA_PRESSAO dev_pressao, ESTRUTURA_Q dev_q, ESTRUTURA_L dev_l ){
	...
	//dev_pressao.p_old[tid] = dev_pressao.p[tid];cuMemcpy
...
// 	dim3 thread(16, 16);
	...
	while (i < 3){
		escoamento_monofasico<<<block, thread>>>(	dev_q, dev_l, dev_b, dev_pressao, dev_mat,
													dev_aux_1, 1e-5, dev_erro, dev_media);
		
		cudaDeviceSynchronize();
		
//		HANDLE_ERROR( cudaMemcpy( dev_media, dev_pressao.p, tam_mat_real * tam_mat_real * sizeof(float),
//                              cudaMemcpyDeviceToDevice ) );
// 		
// 		cudaDeviceSynchronize();
		
		reduce1<<<4, 64>>>( dev_pressao.p, dev_media, 100 );//verificar a melhor escolha para o kernel
		
		cudaDeviceSynchronize();
		
		HANDLE_ERROR( cudaMemcpy( host_b.R, dev_media, tam_mat_real * tam_mat_real * sizeof(float),
		...
		float teste = (float)(host_b.R[0] + host_b.R[1]) / (tam_mat_interna * tam_mat_interna);
		
		HANDLE_ERROR( cudaMemcpy( host_b.R, dev_pressao.p, tam_mat_real * tam_mat_real * sizeof(float),
                              cudaMemcpyDeviceToHost ) );
			printf("\n\n\t\t\t\ti = %d - matriz pressao sem a media\n", i);
			for(k = 0; k < tam_mat_real; k ++){
				for(j = 0; j < tam_mat_real; j++)
					printf("%11.3E ", host_b.R[k*tam_mat_real + j]);
				printf("\n");
			}
		
		perapara_criterio_convergencia<<<block, thread>>>( 	dev_aux_1, 
															dev_aux_2,
															dev_media, 
															dev_q, 
															dev_l, 
		...
		for(k = 0; k < tam_mat_real; k ++){
			for(j = 0; j < tam_mat_real; j++)
				printf("%12.4E ", host_b.R[k*tam_mat_real + j]);
			printf("\n");
		}
