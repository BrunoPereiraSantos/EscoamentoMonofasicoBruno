...
/* - - - - - - - Ponteiros para GPU  - - - - - - - */
float *host_aux_1 = NULL, *dev_aux_1 = NULL, *dev_aux_2 = NULL, dev_erro = NULL, *dev_media =  NULL;
// float *dev_aux_1 = NULL, dev_erro = 0.0, dev_media = 0.0, dev_sum1 = 0.0, dev_sum2 = 0.0;
// 
...
//	int offset = (blockDim.x * gridDim.x) + 1 + 2; // o kernel contem somente a quantidade de elementos internos
	...
	int index_mem_uper = index_mem_central - offset;
	int index_mem_down = index_mem_central + offset;
	int index_mem_left = index_mem_central - 1;
	int index_mem_right = index_mem_central + 1;
	...
	if(tid == eq_tid_cant_sup_esq){//canto superior esquerdo
		...
		*/
		...
		dev_pressao.p[index_mem_central] = (dev_mat.font[index_mem_central] + DR + DD) / (auxR + auxD);
	...
	if(tid == eq_tid_cant_sup_dir){//canto superior direito
		...
		dev_pressao.p[index_mem_central] = (dev_mat.font[index_mem_central] + DD + DL) / (auxD + auxL);
	...
	if(tid == eq_tid_cant_inf_esq){//canto inferior esquerdo
		...
		dev_pressao.p[index_mem_central] = (dev_mat.font[index_mem_central] + DU + DR) / (auxU + auxR);
	...
	if(tid == eq_tid_cant_inf_dir){//canto inferior direito
		...
		dev_pressao.p[index_mem_central] = (dev_mat.font[index_mem_central] + DU + DL) / (auxU + auxL);
	...
	if((tid > eq_tid_cant_sup_esq) && (tid < eq_tid_cant_sup_dir)){//fronteira superior
		...
		dev_pressao.p[index_mem_central] = (dev_mat.font[index_mem_central] + DR + DD + DL) / (auxR + auxD + auxL);
	...
	if((tid > eq_tid_cant_sup_dir) && (tid < eq_tid_cant_inf_dir) && (tid % dimensao_x == dimensao_x - 2)){ //fronteira direita
		...
		dev_pressao.p[index_mem_central] = (dev_mat.font[index_mem_central] + DU + DD + DL) / (auxU + auxD + auxL);
	...
	if((tid > eq_tid_cant_inf_esq) && (tid < eq_tid_cant_inf_dir)){ //fronteira inferior
		...
		dev_pressao.p[index_mem_central] = (dev_mat.font[index_mem_central] + DU + DR + DL) / (auxU + auxR  + auxL);
	...
	if((tid > eq_tid_cant_sup_esq) && (tid < eq_tid_cant_inf_dir) && (tid < eq_tid_cant_inf_esq) && (tid % dimensao_x == 1)){//fronteira esquerda
		...
		dev_pressao.p[index_mem_central] = (dev_mat.font[index_mem_central] + DU + DR + DD) / (auxU + auxR + auxD);
...
__global__ void preapara_criterio_convergencia(float *dev_aux_1, 
											   float *dev_aux_2,
											   float *dev_media, 
											   ESTRUTURA_Q dev_q, 
											   ESTRUTURA_L dev_l, 
	...
	int canto_sup_dir = dimensao_x - 1;
	int canto_inf_dir = (dimensao_x * dimensao_y ) - 1;
	int canto_inf_esq = (dimensao_x * dimensao_y ) - (dimensao_x - 1);
	
	
	
	dev_pressao.p[tid] -= media;
	dev_l.D[tid] -= media;
	dev_l.U[tid] -= media;
	dev_l.L[tid] -= media;
	dev_l.R[tid] -= media;
	
	dev_aux_1[tid] = dev_pressao.p[tid] - dev_pressao.p_old[tid];
	dev_aux_1[tid] = dev_aux_1[tid] * dev_aux_1[tid];
	
	dev_aux_2[tid] = dev_pressao.p[tid] * dev_pressao.p[tid];
	
	//despresando valores do contorno
	if((tid >= 0) && (tid <= canto_sup_dir)){
		dev_pressao.p[tid] = 0.0;
		dev_l.D[tid] = 0.0;
		dev_l.U[tid] = 0.0;
		dev_l.L[tid] = 0.0;
		dev_l.R[tid] = 0.0;
	...
	if((tid > canto_sup_dir) && (tid <= canto_inf_esq) && (tid % dimensao_x == 0)){
		dev_pressao.p[tid] = 0.0;
		dev_l.D[tid] = 0.0;
		dev_l.U[tid] = 0.0;
		dev_l.L[tid] = 0.0;
	...
	if((tid > canto_sup_dir) && (tid < canto_inf_esq) && (tid % dimensao_x == (dimensao_x -1))){
		dev_pressao.p[tid] = 0.0;
		dev_l.D[tid] = 0.0;
		dev_l.U[tid] = 0.0;
		dev_l.L[tid] = 0.0;
	...
	if((tid >= canto_inf_esq) && (tid <= canto_inf_dir)){
		dev_pressao.p[tid] = 0.0;
		dev_l.D[tid] = 0.0;
		dev_l.U[tid] = 0.0;
		dev_l.L[tid] = 0.0;
...
// 	dev_pressao.p_old[tid] = dev_pressao.p[tid];
...
__global__ void teste( ESTRUTURA_PRESSAO dev_pressao, ESTRUTURA_Q dev_q, ESTRUTURA_L dev_l ){
...
__global__ void zeraValores(float *dev_media){
...
__global__ void mapeia_celulas(float *in){
	...
	if(tid == eq_tid_cant_inf_esq){
		in[index_mem_central] = eq_tid_cant_inf_esq;
		in[index_mem_uper] = 2.0;
		in[index_mem_down] = 3.0;
		in[index_mem_left] = 4.0;
		in[index_mem_right] = 5.0;
	}
...
int main(void){
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %12.8E  ", host_mat.perm[i*tam_mat_real + j]);
			//printf(" %6.4E  ", host_mat.perm[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %12.8E  ", host_mat.font[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %12.8E  ", host_mat.epsilon[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	HANDLE_ERROR( cudaMemset( dev_media, 0.0, tam_mat_real * tam_mat_real * sizeof(float) ) );

	mapeia_celulas<<<block, thread>>>(dev_media);

			
	HANDLE_ERROR( cudaMemcpy( host_aux_1, dev_media, tam_mat_real * tam_mat_real * sizeof(float),
                              cudaMemcpyDeviceToHost ) );
	int k;
	for(k = 0; k < tam_mat_real; k ++){
		for(j = 0; j < tam_mat_real; j++){
			printf("%12.8E ", host_aux_1[k*tam_mat_real + j]);
		}
		printf("\n");
	}
	...
	/*
		 ...
		 *	Param: 	ESTRUTURA_Q dev_q,
					...
					ESTRUTURA_PRESSAO dev_pressao,
					ESTRUTURA_MAT dev_mat,
					float *dev_aux_1, const float erro_max
		 *
		 */
	...
	int num_blocks, qtElementos;
	qtElementos = tam_mat_real*tam_mat_real;
	num_blocks = ((qtElementos) > 128) ? (qtElementos/128) + 1 : 1;
	...
	while (i < 10){
		...
		reduce1<<<num_blocks, 128>>>( dev_pressao.p, dev_media, qtElementos );
		...
		for(k = 0; k < tam_mat_real; k ++){
			for(j = 0; j < tam_mat_real; j++){
					printf("%5.10E ", host_aux_1[k*tam_mat_real + j]);
			}
			printf("\n");
		}
		...
		for(k = 0; k < num_blocks; k++){
			somaCuda += host_aux_1[k];
		}
		
		printf("\n  i =  %d \t somaCuda = %f\t mediaCuda = %f\n\n", i, somaCuda, somaCuda/64);
		
		
		HANDLE_ERROR( cudaMemcpy( host_aux_1, dev_pressao.p, tam_mat_real * tam_mat_real * sizeof(float),
                              cudaMemcpyDeviceToHost ) );
		
		
		float somaSerial = 0.0;
		for(k = 0; k < tam_mat_real; k ++){
			for(j = 0; j < tam_mat_real; j++){
				somaSerial += host_aux_1[k*tam_mat_real + j];
			}
		}
		
		printf("somaSerial = %f\t mediaSerial = %f\n\n", somaSerial, somaSerial/64);
		...
		preapara_criterio_convergencia<<<block, thread>>>( dev_aux_1, 
															dev_aux_2,
															dev_media, 
															dev_q, 
															dev_l, 
		...
		HANDLE_ERROR( cudaMemcpy( dev_pressao.p_old, dev_pressao.p, tam_mat_real * tam_mat_real * sizeof(float),
								cudaMemcpyDeviceToDevice ) );
		
		HANDLE_ERROR( cudaMemcpy( dev_q.U_old, dev_q.U, tam_mat_real * tam_mat_real * sizeof(float),
								cudaMemcpyDeviceToDevice ) );
		HANDLE_ERROR( cudaMemcpy( dev_q.R_old, dev_q.R, tam_mat_real * tam_mat_real * sizeof(float),
								cudaMemcpyDeviceToDevice ) );
		HANDLE_ERROR( cudaMemcpy( dev_q.D_old, dev_q.D, tam_mat_real * tam_mat_real * sizeof(float),
								cudaMemcpyDeviceToDevice ) );
		HANDLE_ERROR( cudaMemcpy( dev_q.L_old, dev_q.L, tam_mat_real * tam_mat_real * sizeof(float),
								cudaMemcpyDeviceToDevice ) );
		
		HANDLE_ERROR( cudaMemcpy( dev_l.U_old, dev_l.U, tam_mat_real * tam_mat_real * sizeof(float),
								cudaMemcpyDeviceToDevice ) );
		HANDLE_ERROR( cudaMemcpy( dev_l.R_old, dev_l.R, tam_mat_real * tam_mat_real * sizeof(float),
								cudaMemcpyDeviceToDevice ) );
		HANDLE_ERROR( cudaMemcpy( dev_l.D_old, dev_l.D, tam_mat_real * tam_mat_real * sizeof(float),
								cudaMemcpyDeviceToDevice ) );
		HANDLE_ERROR( cudaMemcpy( dev_l.L_old, dev_l.L, tam_mat_real * tam_mat_real * sizeof(float),
								cudaMemcpyDeviceToDevice ) );
		
		zeraValores<<<block, thread>>>(dev_media);
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_mat.font[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_mat.epsilon[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_b.U[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_b.R[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_b.L[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_b.D[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_q.U[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_q.R[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_q.L[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_q.D[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_l.U[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_l.R[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_l.L[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf(" %6.4E  ", host_l.D[i*tam_mat_real + j]);
		printf("\n");
	}
	...
	for(i = 0; i < tam_mat_real; i ++){
		for(j = 0; j < tam_mat_real; j++)
			printf("%16.6E ", host_pressao.p[i*tam_mat_real + j]);
		printf("\n");
	}*/
...
// 	for(i = 0; i < tam_mat_real; i ++){
// 		for(j = 0; j < tam_mat_real; j++)
// 			printf("%16.6E ", host_pressao.p_old[i*tam_mat_real + j]);
// 		printf("\n");
// 	}
...
float *aloca_matriz(int L, int C){
	float *aux = NULL;
	int comp = L * C;
	//aux = (float *) calloc(comp, sizeof(float));
	aux = (float *) malloc ( comp * sizeof(float));
	if(aux == NULL){
...
char inicializa_parametros(){
	...
	HANDLE_ERROR( cudaMalloc( (void**)&dev_aux_1, tam_mat_real * tam_mat_real * sizeof(float) ) );
	HANDLE_ERROR( cudaMalloc( (void**)&dev_aux_2, tam_mat_real * tam_mat_real * sizeof(float) ) );
	HANDLE_ERROR( cudaMemset( dev_aux_1, 0.0, tam_mat_real * tam_mat_real * sizeof(float) ) );
	HANDLE_ERROR( cudaMemset( dev_aux_2, 0.0, tam_mat_real * tam_mat_real * sizeof(float) ) );
	...
	host_aux_1 = aloca_matriz(tam_mat_real, tam_mat_real);
	
	/**
	*
	*
	*Construir a parte de opÃÃo de contorno
	*
	*
	*/
