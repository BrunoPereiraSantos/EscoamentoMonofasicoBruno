...
__global__ void reducao(float *in){
...
__global__ void reducao2(float *in_1, float *in_2){
...
//	int offset = (blockDim.x * gridDim.x) + 1 + 2; // o kernel contem somente a quantidade de elementos internos
	...
	//dev_media[tid] = dev_pressao.p[tid];
	
		/*
		
		dev_media[0] = reducao(dev_media, 100);
		dev_media[0] = dev_media[0] / (dimensao_x * dimensao_y);
		...
		/*dev_aux_1[index_mem_central] = dev_pressao.p[index_mem_central] - dev_pressao.p_old[index_mem_central];
		__syncthreads();
		
		dev_aux_1[index_mem_central] = dev_aux_1[index_mem_central] * dev_aux_1[index_mem_central];
		__syncthreads();*/
		
		//redução da primeira soma sum1
		
		/*dev_sum1 = reducao(dev_aux_1, 100);*/
		
		//redução da segunda soma sum2
		/*dev_aux_1[index_mem_central] = dev_pressao.p[index_mem_central] * dev_pressao.p[index_mem_central];
		
		__syncthreads();
		
		dev_sum2 = reducao(dev_aux_1, 100);
		
		dev_erro = sqrt(dev_sum1 / dev_sum2);*/
		//DUVIDA PARA COMO É O SINAL DO ERRO
...
// 	dimensao_y -= 2;
	...
	//float media = (dev_media[0] + dev_media[1]) / ((dimensao_x-2) * (dimensao_y-2));
...
// 				printf("%11.3E ", host_aux_1[k*tam_mat_real + j]);
// 			printf("\n");
// 		}
// 		
// 		cudaDeviceSynchronize();
		...
		num_blocks = ((qtElementos) > 512) ? (qtElementos/512) + 1 : 1;
		
		
		
		reduce1<<<num_blocks, 512>>>( dev_pressao.p, dev_media, qtElementos );//verificar a melhor escolha para o kernel
...
// 		}
		...
		M = M / (tam_mat_interna * tam_mat_interna);
...
// 		reducao<<<block, thread>>>( dev_aux_1 );
// 		
// 		cudaDeviceSynchronize();
// 		
// 		reducao<<<block, thread>>>( dev_aux_2 );
// 		
// 		
// 		cudaDeviceSynchronize();
