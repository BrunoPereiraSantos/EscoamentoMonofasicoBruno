...
__device__ int reducao(float *in, int n){
...
//	int offset = (blockDim.x * gridDim.x) + 1 + 2; // o kernel contem somente a quantidade de elementos internos
	...
	while(i < 2){
		...
		dev_aux[index_mem_central] = dev_aux[index_mem_central] * dev_aux[index_mem_central];
		__syncthreads();
		
		//redução da primeira soma sum1
		
		dev_sum1 = reducao(dev_aux, 100);
		
		//redução da segunda soma sum2
		dev_aux[index_mem_central] = dev_pressao.p[index_mem_central] * dev_pressao.p[index_mem_central];
		
		__syncthreads();
		
		dev_sum2 = reducao(dev_aux, 100);
